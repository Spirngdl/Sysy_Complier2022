...
void one_block_use_def (char* use[], char* def[], int size, Block block) {
    ...
    while (p != NULL) {
        ...
        if (p->op == TOK_RETURN) {
            if (p->result.kind == ID) {
                if (!id_exist_strs(p->result.id, use, def, size) && search_alias(p->result.id)) {
        ...
        if (p->op == JLT || p->op == JLE || p->op == JGT || p->op == JGE || p->op == EQ || p->op == NEQ ) {
            if (p->opn1.kind == ID) {
                if (!id_exist_strs(p->opn1.id, use, def, size) && search_alias(p->opn1.id)) { 
                use[use_num] = p->opn1.id;
                use_num++;
                }
            }
            if (p->opn2.kind == ID) {
                if (!id_exist_strs(p->opn2.id, use, def, size) && search_alias(p->opn2.id)) {
